...
class BillingAddressSerializer(serializers.ModelSerializer):
    ...
    postal_code = serializers.CharField(
        required=True, min_length=4, max_length=8)
    cif = serializers.CharField(max_length=20)

    class Meta:
        model = BillingAddress
        fields = ['slug', 'phone_number', 'company_name', 'address', 'city',
                  'province', 'postal_code', 'cif', 'created_at', "email", "country"]


class ShippingAddressSerializer(serializers.ModelSerializer):
    class Meta:
        model = ShippingAddress
        fields = ['slug', 'phone_number', 'name', 'address', 'city',
                  'province', 'postal_code', 'created_at', 'country', "default"]
        many = True
...
class UserProfileSerializer(serializers.ModelSerializer):
    class Meta:
        model = UserProfile
        fields = ['slug', 'is_email_verified', 'cookies_accepted',
                  'cookies_version', 'role', 'language', 'phone_number']
...
class UserSerializer(serializers.ModelSerializer):
    ...
    user_shipping_address = ShippingAddressSerializer(
    ...
    class Meta:
        ...
        fields = ['id', 'first_name', 'last_name', 'username', 'email', 'password', 'confirm_password',
                  'is_active', 'date_joined', 'last_login', 'user_profile', 'user_shipping_address', 'user_billing_address']
    ...
    def create(self, validated_data):
        ...
        user_profile = UserProfile.objects.update(
            auth_user=user, **user_profile_data)
