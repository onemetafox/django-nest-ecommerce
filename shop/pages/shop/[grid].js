...
function ShopGrid() {
  ...
  async function getProducts(variables) {
    await fetcher(
      'web/search?' + new URLSearchParams({ ...variables }).toString(),
    )
      .then(res => {
        setData(res);
      })
  ...
  return (
    <main className="main">
      {/* TODO:: ADD BANNER FRONT BACK END */}
      <ShopBanner />
        ...
        <nav aria-label="breadcrumb" className="breadcrumb-nav">
          <ol className="breadcrumb">
            ...
            {query.category ? (
              <>
                <li className="breadcrumb-item">
                  <ALink href={`/${t('SHOP')}/${grid}`} scroll={false}>
                    {t('SHOP')}
                  </ALink>
                </li>
                {data &&
                  data.products.categoryFamily.map((item, index) => (
                    ...
                    >
                      <ALink
                        href={`/${t('SHOP')}/${grid}?category=${item.slug}`}
                        scroll={false}
                      >
                        {item.name}
                      </ALink>
