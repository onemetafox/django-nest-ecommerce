...
class HomeBanner(models.Model):
    ...
    banner_image = models.ImageField(upload_to=get_banner_image_upload_path, validators=[
    ...
    def save(self, *args, **kwargs):
        if not self.slug:
            slug_text = slugify(self.name)
            self.slug = unique_slugify(self, slug_text)
            self.image_url = self.banner_image.url
        self.clean()
        super().save(*args, **kwargs)
...
class HomeSlider(models.Model):
    slug = models.SlugField(max_length=250, null=False,
                            blank=False, editable=False, allow_unicode=True)
    slider_image = models.ImageField(upload_to=get_banner_image_upload_path, validators=[
        FileExtensionValidator(['png', 'jpg', 'jpeg'])])
    image_url = models.CharField(
        max_length=250, null=True, blank=True, default=None)
    ...
    def save(self, *args, **kwargs):
        if not self.slug:
            ...
            self.slug = unique_slugify(self, slug_text)
            self.image_url = self.slider_image.url
        self.clean()
        super().save(*args, **kwargs)
